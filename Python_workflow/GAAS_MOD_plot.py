...
fig = plt.figure()
X, Y = np.meshgrid(gold_gap*1e6, algaas_gap*1e6)
Z = v_pi_l
ax = plt.axes(projection='3d')
ax.plot_surface(X, Y, Z, rstride=1, cstride=1,
                cmap='viridis', edgecolor='none')
ax.set_title('VpiL vs (Side Gap, Top Gap)')
ax.set_xlabel('Distance to side electrode (um)')
ax.set_ylabel('Distance to top electrode (um)')
